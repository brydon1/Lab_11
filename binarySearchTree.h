...
class bSearchTreeType: public binaryTreeType<int>
{
public:
...
void bSearchTreeType<elemType>::deleteNode(const elemType& deleteItem)
{
    ...
    else
    {
        ...
        if (current == NULL)
         ...
         //else if (found)
            //deleteFromTree(current);  //alternative to the code below ?
...
void bSearchTreeType<elemType>::deleteFromTree (binaryTreeNode<elemType>* &p)
{
    ...
    else if(p->llink == NULL) //node has no left child (or left subtree)
    {
        temp = p;
        p = temp->rlink;
        delete temp;
    }
    else if(p->rlink == NULL) //node has no right child (or right subtree)
    {
        temp = p;
        p = temp->llink;
        delete temp;
    }
    else                    //node has two children, what exactly happens in this case?
    {
        current = p->llink;
        trailCurrent = NULL;
